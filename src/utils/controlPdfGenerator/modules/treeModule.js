...
const renderCorporateLocationSection = async (pdfState, locationNode, remarks, yPos, design, imageCache = null) => {
  ...
  // CLEAN LOCATION HEADER med svart cirkel - större och tjockare
  ...
  // Rita svart cirkel före namnet - centrerad med texten
  doc.setFillColor(0, 0, 0); // Svart
  doc.circle(margins.left + 3, yPos - 1.5, 2, 'F'); // Cirkel med radie 2px, centrerad
  
  doc.text(locationNode.name, margins.left + 10, yPos); // Offset texten för cirkeln
...
const renderCorporateAreaSection = async (pdfState, areaNode, remarks, yPos, design, level = 1, imageCache = null) => {
  ...
  // SIMPLE AREA HEADER med streck - tjockare text för tydlighet
  const indentX = margins.left + (level * 12); // 12px indrag per nivå (uppdaterat från design)
  
  doc.setTextColor(...design.colors.secondary);
  doc.setFont('helvetica', design.fonts.areaName.weight);
  doc.setFontSize(design.fonts.areaName.size);
  
  // Rita streck före namnet
  doc.text('–', indentX, yPos); // Em-dash symbol
  
  doc.text(areaNode.name, indentX + 8, yPos); // Offset texten för strecket
